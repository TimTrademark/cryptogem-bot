...
logging.basicConfig(
    format="[%(asctime)s] - %(message)s", datefmt="%d-%m-%y %H:%M", level=logging.INFO
)
...
def main():
    ...
    config_loader.append_add_callback(
        lambda: init_last_non_tradable(config_loader.connectors, last_non_tradable)
    )
    ...
    while True:
        try:
            ...
            for connector in connectors:
                ec: ExchangeConfig = config.get_exchange_config_by_name(
                    connector.get_connector_name()
                )
                if not ec.active:
                    continue
                latest_pairs = connector.get_latest_pairs()
                compare_latest_pairs_and_trade(
                    connector,
                    latest_pairs,
                    ec.funds,
                ...
                last_non_tradable[connector.get_connector_name().lower()] = (
                    PairUtils.filter_non_tradable_pairs(latest_pairs)
                )
...
def init_last_non_tradable(
    ...
    for c in connectors:
        last_non_tradable[c.get_connector_name().lower()] = (
            PairUtils.filter_non_tradable_pairs(c.get_latest_pairs())
        )
